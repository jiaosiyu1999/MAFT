    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor, newhw: torch.Tensor = None):
        if self.attn_mask is not None:
            ...
            if mask is not None:
                ...
                attn_mask = (mask.flatten(2).unsqueeze(1).repeat(1, self.num_heads, 1, 1).flatten(0, 1))  # bH*100*hw (H: num_head)
                if self.mask_pool is None or (self.mask_pool.shape[0] != attn_mask.shape[0]):
                    self.mask_pool = torch.eye(attn_mask.shape[1]).unsqueeze(0).repeat(attn_mask.shape[0],1,1).to(attn_mask.device)  # maskpool: bH*100*100
                ...
                if self.mask_poolhw is None or (self.mask_poolhw.shape[0] != attn_mask.shape[0]):
                    self.mask_poolhw = torch.ones(attn_mask.shape[0], attn_mask.shape[2] - attn_mask.shape[1], attn_mask.shape[2]).to(attn_mask.device) # mask_poolhw: bH*hw*100+hw
